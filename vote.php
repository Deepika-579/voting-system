...
$select="select * from elections_tb";
...
if(isset($_POST['search_candidate'])){
	...
	$select="select * from elections_tb where elections_name='$elections_name'";
	$run=$conn->query($select);
	if($run->num_rows>0){
		while($row=$run->fetch_array()){
	...
	$current_ts=time();
	 ...
	 if($elections_start_date_ts>$current_ts){
		 echo "Election Didnot Start Please Wait....";
	 }
	 else if($current_ts>$elections_end_date_ts){
		 echo "Election Has Been Closed You Didnot Caste Your Vote";
		 
	 }
	 else{
		 ?>
		<a href="votecaste.php?elections_name=<?php echo str_replace('','-',$elections_name);?>" style="color:green">Click Here</a>
		 <?php
	 }
}
?>
